"""Thumbs+sheet for _cands5 -> _sheets5/_vs5."""
...
os.makedirs("_thumbs5", exist_ok=True)
os.makedirs("_sheets5", exist_ok=True)
os.makedirs("_vs5", exist_ok=True)
font = ImageFont.truetype("arial.ttf", 13)
TH_W, TH_H, COLS = 200, 260, 5

tasks = []
for f in sorted(os.listdir("_cands5")):
    slug = f[:-5]
    cands = json.load(open(f"_cands5/{f}", encoding="utf-8"))
    os.makedirs(f"_thumbs5/{slug}", exist_ok=True)
...
for i, slug, _, _ in tasks:
    if os.path.exists(f"_thumbs5/{slug}/{i}.jpg"):
...
for slug, ids in by_slug.items():
    ...
    for i in sorted(ids):
        try:
            im = Image.open(f"_thumbs5/{slug}/{i}.jpg").convert("RGB")
    ...
    sheet.save(f"_sheets5/{slug}.jpg", quality=80)
    im = Image.open(f"_sheets5/{slug}.jpg")
    W, H = im.size
    rows_total = -(-H // TH_H)
    half = -(-rows_total // 2)
    for part, (r0, r1) in enumerate([(0, half), (half, rows_total)]):
        ...
        crop.save(f"_vs5/{slug}__{part+1}.jpg", quality=82)

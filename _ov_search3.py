"""Third-pass search for the 7 slugs with no/weak candidates. Writes _cands3/<slug>.json (fresh files)."""
...
queries = json.load(open("_queries3.json", encoding="utf-8"))
os.makedirs("_cands3", exist_ok=True)
for slug, qs in queries.items():
    results = []
    for q in qs:
        for page in (1, 2):
            u = API + "?" + urllib.parse.urlencode({"q": q, "page_size": 20, "mature": "false", "page": page})
            d = fetch(u)
            if not d:
                continue
            ...
            time.sleep(0.7)
    json.dump(results, open(f"_cands3/{slug}.json", "w", encoding="utf-8"), indent=1)
